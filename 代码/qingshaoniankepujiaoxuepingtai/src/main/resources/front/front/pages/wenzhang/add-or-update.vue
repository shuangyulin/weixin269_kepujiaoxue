        ...
        <form class="app-update-pv">
            <view
                ...
                <input   disabled
                         ...
                         :disabled="ro.wenzhangName" type="text" v-model="ruleForm.wenzhangName" placeholder="文章标题"></input>
            </view>
                <view :style='{"boxShadow":"0","backgroundColor":"rgba(255, 255, 255, 0.25)","borderColor":"rgba(228, 232, 230, 1)","margin":"0 0 0px 0","borderWidth":"2rpx","borderStyle":"none none solid none ","height":"88rpx"}'
                      class="cu-form-group">
                    <view :style='{"width":"160rpx","fontSize":"28rpx","color":"#47EDB5","textAlign":"left"}'
                          class="title">文章类型</view>
                    <picker @change="wenzhangTypesChange" :value="wenzhangTypesIndex" :range="wenzhangTypesOptions" range-key="indexName">
                        <view
                                :style='{"boxShadow":"0 0 0px rgba(0,0,0,.6) inset","backgroundColor":"rgba(255, 255, 255, 1)","borderColor":"#47EDB5","borderRadius":"16rpx","color":"#47EDB5","textAlign":"left","borderWidth":"2rpx","fontSize":"28rpx","borderStyle":"solid","height":"88rpx","marginTop":"7rpx"}'
                                class="uni-input">{{ruleForm.wenzhangTypes?ruleForm.wenzhangValue:"请选择文章类型"}}</view>
                    </picker>
                </view>
                <view
                        ...
                        class="cu-form-group" @tap="wenzhangPhotoTap">
                    <view :style='{"width":"160rpx","fontSize":"28rpx","color":"#47EDB5","textAlign":"left"}'
                          class="title">文章封面</view>
                    <view class="right-input" style="padding:0;textAlign:left">
                        <image
                                ...
                                class="avator" v-if="ruleForm.wenzhangPhoto" :src="ruleForm.wenzhangPhoto" mode="aspectFill">
                        </image>
                        <image  :style='{"width":"68rpx","boxShadow":"0 0 0px rgba(0,0,0,.3)","borderRadius":"100%","textAlign":"left","height":"68rpx"}'
                                class="avator" v-else src="../../static/gen/upload.png" mode="aspectFill"></image>
					                ...
					                <view
                        :style='{"boxShadow":"0 0 0px rgba(0,0,0,.3)","backgroundColor":"rgba(255, 255, 255, 0.25)","borderColor":"rgba(218, 220, 219, 1)","margin":"0 0 0px 0","borderWidth":"2rpx","borderStyle":"none none solid none ","height":"108rpx"}'
                        class="cu-form-group" @tap="wenzhangFileTap">
                    <view :style='{"width":"160rpx","fontSize":"28rpx","color":"#47EDB5","textAlign":"left"}'
                          class="title">文章</view>
                    <view class="right-input" style="padding:0;textAlign:left">
                        <image
                                :style='{"width":"68rpx","boxShadow":"0 0 0px rgba(0,0,0,.3)","borderRadius":"100%","textAlign":"left","height":"68rpx"}'
                                class="avator" v-if="ruleForm.wenzhangFile" :src="ruleForm.wenzhangFile" mode="aspectFill">
                        </image>
                        <image  :style='{"width":"68rpx","boxShadow":"0 0 0px rgba(0,0,0,.3)","borderRadius":"100%","textAlign":"left","height":"68rpx"}'
                                class="avator" v-else src="../../static/gen/upload.png" mode="aspectFill"></image>
                    </view>
...
<!-- text后缀 -->
            <view
                ...
                <textarea :style='{"boxShadow":"0 0 0px rgba(0,0,0,.6) inset","backgroundColor":"rgba(255, 255, 255, 1)","borderColor":"#47EDB5","borderRadius":"16rpx","color":"#47EDB5","textAlign":"left","borderWidth":"2rpx","fontSize":"28rpx","borderStyle":"solid","height":"88rpx","marginTop":"7rpx","height":"200rpx"}'
                          :disabled="ro.wenzhangContent" v-model="ruleForm.wenzhangContent" placeholder="文章内容"/>
            </view>
...
<script>
    ...
    export default {
		...
		/*下拉框*/
			let wenzhangTypesParams = {
                page: 1,
                limit: 100,
                dicCode: 'wenzhang_types',
            }
			let wenzhangTypes = await this.$api.page(`dictionary`, wenzhangTypesParams);
			this.wenzhangTypesOptions = wenzhangTypes.data.list
            ...
            if (options.id) {
                ...
                // 获取信息
                let res = await this.$api.info(`wenzhang`, this.ruleForm.id);
        ...
        methods: {
            // 下拉变化
            wenzhangTypesChange(e) {
                this.wenzhangTypesIndex = e.target.value
                this.ruleForm.wenzhangValue = this.wenzhangTypesOptions[this.wenzhangTypesIndex].indexName
                this.ruleForm.wenzhangTypes = this.wenzhangTypesOptions[this.wenzhangTypesIndex].codeIndex
            },
			...
			wenzhangPhotoTap() {
                ...
                this.$api.upload(function(res) {
                    _this.ruleForm.wenzhangPhoto = _this.$base.url + 'upload/' + res.file;
                    _this.$forceUpdate();
                    _this.$nextTick(() => {
                        // _this.styleChange()
                    })
			...
			wenzhangFileTap() {
                let _this = this;
                this.$api.upload(function(res) {
                    _this.ruleForm.wenzhangFile = _this.$base.url + 'upload/' + res.file;
                    _this.$forceUpdate();
                    _this.$nextTick(() => {
                        // _this.styleChange()
                    })
            ...
            async onSubmitTap() {
				if ((!this.ruleForm.wenzhangName)) {
                    this.$utils.msg(`文章标题不能为空`);
                    return
                }
				if ((!this.ruleForm.wenzhangPhoto)) {
                    this.$utils.msg(`文章封面不能为空`);
                    return
                }
                ...
                if (this.ruleForm.id) {
                    ...
                    await this.$api.save(`wenzhang`, this.ruleForm);
                }
